# Hi there, I'm MrA 👋

> 🌙 `newbie h4ck3r by night` | ☀️ `regular Student by day`
> In hacker circles, my alias is `Zero` | The student ID is `6811704177362`

Welcome to my GitHub profile - [@MrAngRea1](https://github.com/MrAngRea1). I'm a student who loves learning how things work, from websites to AI to cybersecurity.

---
<img align="right" alt="Avatar" width="180" src="https://mra.free.nf/assets/avatar.png">

### 🔭 What I'm into

- 🛡️ **Cybersecurity** - just starting out, learning basics + CTF mindset
- 💻 **Programming** - building small projects to learn by doing
- 🤖 **AI / LLM** - experimenting with local LLMs & prompts
- 📚 **Student life** - learning a little bit every day

> I'm still a beginner - but consistent. `learn → build → break → fix → repeat`

---

### 📫 How to reach me

- GitHub: [@MrAngRea1](https://github.com/MrAngRea1)
